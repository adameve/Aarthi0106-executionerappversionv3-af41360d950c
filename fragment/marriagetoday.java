...
public class marriagetoday extends Fragment {

    public static marriagetoday thisToday = null;
    String year, month, day, hour, min, id, sdate, owner, tenant, w1, w2, rkey, timevalue, timevalue1 = "", not_id, from = "Today", biocontent = "", content = "", o_content = "", t_content = "", w1_content = "", w2_content = "",
    ...
    int year1, month1,t_hour1=0,t_min1=0, day1, hour1, h2, h3, min1, click, repeat, intervalvalue, id1, count = 0, flag;
                        ...
                        if (sw1.isChecked()) {
                            ...
                            b1.setOnClickListener(new OnClickListener() {
                                ...
                                public void onClick(View v) {
                                    ...
                                    mTimePicker = new TimePickerDialog(marriagetoday.this.getContext(), new TimePickerDialog.OnTimeSetListener() {
                                        @Override
...
//
                R_key.setOnClickListener(new OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        i.putExtra("marriage_type", getAlldataList.get(position).get("marriage_type"));
...
//
            ...
            public void run() {
                if (db.getnotificationtable(db).size() > 0) {
                    ...
                    for (int i = 0; i < db.getnotificationtable(db).size(); i++) {
                            ...
                            if (db.getnotificationtable(db).get(i).get("appid").equals(db.getAllmarriageTodayList(db).get(position).get("appid"))) {
                                ...
                                if (db.getnotificationtable(db).get(i).get("time").equals(curr_time)) {
                                    ...
                                    int count4 = sqldb2.update(DBManager.TableInfo.TABLE_MARRIAGE_APPOINTMENT, values2, DBManager.TableInfo.db_marriage_appointment_id + "=?", new String[]{db.getAllmarriageTodayList(db).get(position).get("appid")});
                                    sqldb2.close();

                                    SQLiteDatabase marriagedocu = db.getWritableDatabase();
                                    marriagedocu.delete(TABLE_NOTIFICATION, DBManager.TableInfo.db_marriage_appointment_id + "=?", new String[]{db.getAllmarriageTodayList(db).get(position).get("appid")});
    ...
    private void viewDetails() {
        ...
        if (GenericMethods.marriagetodayyy.equals("true")) {
            ...
            getAlldataList = GenericMethods.getAllmarriagetodaylist1;
        ...
        } else if (GenericMethods.marriagetodayyy.equals("false")) {
                ...
                for (int i = 0; i < db.getnotificationtable(db).size(); i++) {
                    ...
                    for (int j = 0; j < repeat; j++) {
                        ...
                        if (getAlldataList.get(position).get("appid").equals(db.getnotificationtable(db).get(i).get("appid"))) {
                            System.out.println("appid from doc table :" + getAlldataList.get(position).get("appid") + "appid from notification:" + db.getnotificationtable(db).get(i).get("appid"));
