...
public class biometricappointments extends Fragment {
    ...
    private void setupViewPager(ViewPager viewPager) {
        ...
        if (db.getAllbiometriccancelledappointments(db).size() == 0) {
            adapter.addFragment(new NoAppointmentdetails(), "Cancelled");

        } else if (db.getAllbiometriccancelledappointments(db).size() > 0) {
            adapter.addFragment(new CancelledAppointments(), "Cancelled");

        }
        if (db.getAllbiometricautocancelledappointments(db).size() == 0) {
            adapter.addFragment(new NoAppointmentdetails(), "Auto-Cancelled");

        } else if (db.getAllbiometricautocancelledappointments(db).size() > 0) {
            adapter.addFragment(new AutocancelledAppointments(), "Auto-Cancelled");
